...
#     # TODO: Rewrite this and include backwards compatability with ivps
#     __baked__ = 0
#     __stepFunc__ = []
#     __verbose__ = 0
#
#     def __init__(self, name='ForwardEuler', MechanicalSystem=[], ConstantsOfMotion=[]):
#         raise NotImplementedError
#         self.name = name
#         self.MechanicalSystem = MechanicalSystem
#         self.odesys = MechanicalSystem.odesys
#         self.stepFunc = []
#         self.step = Symbol('tau')
#         self.independent = MechanicalSystem.manifold.independent[0]
#         self.states = self.MechanicalSystem.manifold.coords[:floor(self.MechanicalSystem.manifold.dim / 2)]
#         self.costates = self.MechanicalSystem.manifold.coords[floor(self.MechanicalSystem.manifold.dim / 2):]
#         self.statesFINAL = []
#         self.costatesFINAL = []
#         self.subgators = []
#         consts = ConstantsOfMotion
#         self.constantsofmotion = []
#
#         statelistbad = []
#         for ii in self.states + self.costates:
#             statelistbad.append(Symbol(str(ii)))
#
#         for ii in consts:
#             if isinstance(ii, str):
#                 self.constantsofmotion.append(sympify(ii).subs(dict(zip(statelistbad, self.states + self.costates)), simultaneous=True))
#             else:
#                 self.constantsofmotion.append(ii)
#
#         self.matlab = MatlabInterface()
#
#         possibles = globals().copy()
#         possibles.update(locals())
#         method = possibles.get('gen_' + name)
#         if not method:
#             raise Exception("Method %s not implemented" % name)
#         self.solver = method
#
#     def genStepFunc(self,verbose=0):
#         if self.odesys == []:
#             self.odesys = self.MechanicalSystem.converttoODE(verbose=verbose)
#
#         # Fix the output order of the state rates from the GMS
#         StatesFromRates = [str(ii)[:-4] for ii in self.odesys.states]
#         StatesFromManifold = [str(ii) for ii in self.MechanicalSystem.manifold.coords]
#         StateTransition = [StatesFromManifold.index(ii) for ii in StatesFromRates]
#
#         EquationList = []
#
#         for ii in StateTransition:
#             EquationList.append(self.odesys.equations[ii])
#
#         stateRates = EquationList[:floor(len(EquationList)/2)]
#         costateRates = EquationList[floor(len(EquationList)/2):]
#
#
#         self.statesFINAL = []
#         self.costatesFINAL = []
#         for ss,cc in zip(self.states,self.costates):
#             self.statesFINAL.append(Symbol(str(ss) + '_f'))
#             self.costatesFINAL.append(Symbol(str(cc) + '_f'))
#
#         if verbose >= 0:
#             print('Generating step function... ', end='')
#             sys.stdout.flush()
#
#         self.stepFunc = self.solver.generate_step_function(states=self.states, costates=self.costates, stateRates=stateRates, costateRates=costateRates, dim=self.MechanicalSystem.manifold.dim, dt=self.step, statesFINAL=self.statesFINAL, costatesFINAL=self.costatesFINAL)
#
#         if verbose >= 0:
#             print('Done')
#             sys.stdout.flush()
#
#     def bake(self, verbose=0):
#         if verbose > 0:
#             print('Attempting to solve the ' + self.name + ' method... ', end='')
#             sys.stdout.flush()
#
#         try:
#
#             # dictset = solve(self.stepFunc,self.statesFINAL+self.costatesFINAL, rational=False, quick=True)
#             # keyboard()
#             dictset = self.matlab.solve(equations=self.stepFunc, variables=self.statesFINAL+self.costatesFINAL, auxvars=self.states + self.costates)
#             # keyboard()
#             eqset = [dictset[ii][0] for ii in self.statesFINAL + self.costatesFINAL]
#             # keyboard()
#             self.__stepFunc__ = lambdify((self.step, self.states + self.costates), eqset, "numpy")
#             # keyboard()
#             self.__baked__ = 1
#             if verbose > 0:
#                 print('Done')
#                 sys.stdout.flush()
#         except:
#             print('Error: Could not bake. Continuing numerically')
#             self.__baked__ = 0
#
#     def evalStep(self, dt, x, constvals=[]):
#         xnew = []
#         if self.__baked__ == 1:
#             # Use solved analytical procedure
#             xnew = self.__stepFunc__(dt,x)
#         else:
#             # Solve numerically
#             finaleq = []
#             for index1, equation in enumerate(self.stepFunc):
#                 finaleq.append(equation)
#                 finaleq[-1] = finaleq[-1].subs(dict(zip(self.states+self.costates, [ii for ii in x])), simultaneous=True)
#                 finaleq[-1] = finaleq[-1].subs(self.step, dt)
#
#             # ans = nsolve(finaleq, (self.statesFINAL+self.costatesFINAL), [ii for ii in x])
#             # ans = np.matrix(ans.tolist(), dtype=np.float).transpose()
#             # keyboard()
#             ans = self.matlab.fsolve(equations=finaleq, x0=[ii for ii in x], variables=self.statesFINAL + self.costatesFINAL)
#             ans = np.array([ans[ii] for ii in self.statesFINAL + self.costatesFINAL])
#             xnew = ans
#
#         if len(self.constantsofmotion) > 0:
#             errors = [ii - jj for ii, jj in zip(xnew, self.states+self.costates)]
#             obj_func = 0
#
#             for ii in errors:
#                 obj_func += ii**2
#
#             xproj = self.matlab.fmincon(cost=[obj_func], x0=xnew, A=[], b=[], Aeq=[], beq=[], lb=[], ub=[], nonlinineq=[], nonlineq=[ii - jj for ii, jj in zip(self.constantsofmotion, constvals)], variables=self.states+self.costates)
#             xnew = np.array([xproj[ii] for ii in self.states+self.costates])
#             # keyboard()
#
#         return xnew
#
#     def solve(self, tSpan=[], x0=[]):
#         T = np.array(tSpan)
#         X = np.zeros((len(tSpan),len(x0)))
#         X[0] = np.array(x0)
#         constvals = []
#         for ii in self.constantsofmotion:
#             constvals.append(ii.subs(dict(zip(self.states+self.costates, x0))))
#
#         for ii in range(len(T)-1):
#             print(ii)
#             X[ii+1] = np.array(self.evalStep(T[ii+1]-T[ii], X[ii], constvals=constvals))
#
#         C = Curve(t=T,x=X)
#         return C