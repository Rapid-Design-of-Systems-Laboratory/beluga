...
from beluga.numeric.bvp_solvers import Collocation
import numpy as np
...
def test_T6():
    ...
    def odefun(X, u, p, const):
        return 2 * X[1],\
               2 * ((-X[2] * X[1] - const[0] * np.pi ** 2 * np.cos(np.pi * X[2]) - np.pi * X[2]
                     * np.sin(np.pi * X[2])) / const[0]),\
               2
...
def test_T11(const):
    def odefun(X, u, p, const):
        return 2 * X[1], 2 * ((X[0] - const[0] * np.pi ** 2 * np.cos(np.pi * X[2]) - np.cos(np.pi * X[2]))
                              / const[0]), 2
...
def test_R2(const):
    ...
    algo = Collocation(odefun, quadfun, bcfun)
    solinit = Trajectory()
    solinit.t = np.linspace(0, 1, 2)
