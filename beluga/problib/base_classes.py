...
from beluga.codegen import LocalCompiler
...
class BaseInput(BaseProblem):
    ...
    def sympify_vars(self, sym_prob):
        ...
        # Independent
        self.sympify_name(sym_prob.ind_var)

        # States
        for state in sym_prob.states:
            self.sympify_name(state)

        # Parmeters
        for parameter in sym_prob.parameters:
            self.sympify_name(parameter)

        # Constants
        for constant in sym_prob.constants:
            self.sympify_name(constant)

        # Custom Functions
        for custom_function in sym_prob.custom_functions:
            ...
            custom_function['sym_func'] = custom_functions_lib.CustomFunctionGenerator(
                custom_function['func'], name=custom_function['name'], func_dict=sym_prob.local_compiler.func_locals)
            sym_prob.add_symbolic_local(custom_function['name'], custom_function['sym_func'])

        # Tables
        for table in sym_prob.tables:
            ...
            table['sym_table'] =\
                table_lib.SymTableGenerator(table['table_func'], func_dict=sym_prob.local_compiler.func_locals, order=0)
            sym_prob.add_symbolic_local(table['name'], table['sym_table'])

        # Switches
        for switch in sym_prob.switches:
            self.sympify_name(switch)

        # Quantites TODO Find a more elegant solution to this
        for _ in enumerate(sym_prob.quantities):
            for quantity in sym_prob.quantities:
                quantity['expr'] = sym_prob.local_compiler.sympify(str(quantity['expr']))
                sym_prob.add_symbolic_local(quantity['name'], quantity['expr'])
    ...
    def sympify_units(sym_prob):
        # Units
        for unit in sym_prob.units:
            unit['sym'] = sympy.Symbol(unit['name'])

        # Independent
        sym_prob.ind_var['units'] = sym_prob.sympify(sym_prob.ind_var['units'])

        # States
        for state in sym_prob.states:
            state['units'] = sym_prob.sympify(state['units'])

        # Quads
        for quad in sym_prob.quads:
            quad['units'] = sym_prob.sympify(quad['units'])

        # Parmeters
        for parameter in sym_prob.parameters:
            parameter['units'] = sym_prob.sympify(parameter['units'])

        # Constants
        for constant in sym_prob.constants:
            constant['units'] = sym_prob.sympify(constant['units'])

            # Constraints
            for location in ['initial', 'terminal']:
                for constraint in sym_prob.constraints[location]:
        ...
        # Custom Functions
        for custom_function in sym_prob.custom_functions:
            custom_function['func_units'] = sym_prob.sympify(custom_function['func_units'])
            custom_function['arg_units'] = [sym_prob.sympify(arg_units) for arg_units in custom_function['arg_units']]

        # Tables
        for table in sym_prob.tables:
            table['ret_units'] = sym_prob.sympify(table['ret_units'])
            table['arg_units'] = [sym_prob.sympify(arg_units) for arg_units in table['arg_units']]
    ...
    def sympify_exprs(sym_prob):
        # States
        for state in sym_prob.states:
            state['eom'] = sym_prob.sympify(state['eom'])
        ...
        # Switches
        for switch in sym_prob.switches:
            switch['functions'] = sym_prob.sympify(switch['functions'])
            switch['conditions'] = sym_prob.sympify(switch['conditions'])
            switch['tol'] = sym_prob.sympify(switch['tol'])
    ...
    def sympify_problem(self, sym_prob=None):
        ...
        self.sympify_vars(sym_prob)
        self.sympify_units(sym_prob)
        self.sympify_exprs(sym_prob)

        return sym_prob


class BaseSym(BaseProblem):
    def __init__(self, name=None, local_compiler=None):
        BaseProblem.__init__(self, name=name)
