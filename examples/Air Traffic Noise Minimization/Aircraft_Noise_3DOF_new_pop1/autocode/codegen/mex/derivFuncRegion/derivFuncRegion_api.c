/*
 * derivFuncRegion_api.c
 *
 * Code generation for function 'derivFuncRegion_api'
 *
...
static void b_info_helper(ResolvedFunctionInfo info[95])
{
  ...
  info[90].context =
    "[E]/home/mall/mjgrant/trajProblems/Friends/Janav/Aircraft_Noise_3DOF_new_pop1/autocode/derivFuncRegion.m";
  info[90].name = "cos";
  ...
  msgId))[6]
{
  ...
  void derivFuncRegion_api(const mxArray * const prhs[11], const mxArray *plhs[1])
{
  real_T (*xDotOut)[12];
  real_T t;
  real_T (*X)[12];
  ...
  region = emlrt_marshallIn(emlrtAliasP(prhs[2]), "region");
  e_emlrt_marshallIn(emlrtAlias(prhs[3]), "p", &p_data, p_size);
  g_emlrt_marshallIn(emlrtAliasP(prhs[4]), "const", &b_const);
  i_emlrt_marshallIn(emlrtAliasP(prhs[5]), "constraint");
  arcSequence = emlrt_marshallIn(emlrtAliasP(prhs[6]), "arcSequence");
  k_emlrt_marshallIn(emlrtAlias(prhs[7]), "interiorPointConstraintSequence");
  k_emlrt_marshallIn(emlrtAlias(prhs[8]), "interiorPointNumLagrangeMultipliers");
  x0 = m_emlrt_marshallIn(emlrtAlias(prhs[9]), "x0");
  xf = m_emlrt_marshallIn(emlrtAlias(prhs[10]), "xf");

  /* Invoke the target function */
  derivFuncRegion(t, *X, region, *p_data, p_size, &b_const, arcSequence, *x0,
                  *xf, *xDotOut);
...
/* End of code generation (derivFuncRegion_api.c) */
