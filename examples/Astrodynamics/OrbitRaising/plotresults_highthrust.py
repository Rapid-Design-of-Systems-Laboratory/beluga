...
data = load('highthrust_shooting_data.beluga')
sol_set_shooting = data['solutions']
data = load('highthrust_collocation_data.beluga')
...
plt.figure()
plt.plot(0, 0, marker='*')
plt.plot(np.cos(sol_s.y[:, 1]) * sol_s.y[:, 0], np.sin(sol_s.y[:, 1]) * sol_s.y[:, 0], color='b', linestyle='-',
         linewidth=2,
         label='Shooting')
plt.plot(np.cos(sol_c.y[:, 1]) * sol_c.y[:, 0], np.sin(sol_c.y[:, 1]) * sol_c.y[:, 0], color='r', linestyle='--',
         marker='.',
         label='Collocation')
...
plt.figure()
plt.plot(sol_s.t, sol_s.lam[:, 0], color='b', linestyle='-', label='Radius (costate)')
plt.plot(sol_s.t, sol_s.lam[:, 1], color='r', linestyle='-', label='True Anomaly (costate)')
plt.plot(sol_s.t, sol_s.lam[:, 2], color='g', linestyle='-', label='Radial Velocity (costate)')
plt.plot(sol_s.t, sol_s.lam[:, 3], color='y', linestyle='-', label='Tangential Velocity (costate)')
plt.plot(sol_c.t, sol_c.lam[:, 0], color='b', linestyle='--', marker='.')
plt.plot(sol_c.t, sol_c.lam[:, 1], color='r', linestyle='--', marker='.')
plt.plot(sol_c.t, sol_c.lam[:, 2], color='g', linestyle='--', marker='.')
plt.plot(sol_c.t, sol_c.lam[:, 3], color='y', linestyle='--', marker='.')
plt.xlabel('Time [nd]')
