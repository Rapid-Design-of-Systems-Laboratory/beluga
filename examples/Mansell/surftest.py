...
def get_problem():
    ...
    problem = beluga.optim.Problem('surftest')
    
    #Define independent variables
    problem.independent('t', 's')
    
    # Define equations of motion
    problem.state('x','V*cos(hdg)*cos(inc) + eps*(cos(thrA)+cos(hdgA)+cos(incA))','m')   \
           .state('y','V*sin(hdg)*cos(inc)','m')  \
           .state('z','-V*sin(inc)','m')  \
           .state('V','amax*sin(thrA)-g*sin(inc)','m/s') \
           .state('hdg','cmax/V*sin(hdgA)','rad') \
           .state('inc','cmax/V*sin(incA)','rad')
    ...
    problem.cost['path'] =  Expression('V', 'm')#+ \
    ...
    problem.constraints().initial('x-x_0','m') \
                         ...
                         .initial('z-z_0','m') \
                         .initial('V-V_0','m/s') \
                         .initial('inc-inc_0','rad') \
                         .terminal('x-x_f','m') \
                         .terminal('y-y_f','m') \
                         .terminal('z-z_f','m') \
                         .terminal('V-V_f','m/s') \
                         .terminal('inc-inc_f','rad')
    
    #Define constants
    problem.constant('g',9.81,'m/s^2') #Acceleration due to gravity
    problem.constant('trk',10e3,'USD/m') #Basic cost of 1 m of track on ground
    problem.constant('amax',1.0,'m/s^2') #Maximum thrust acceleration of vehicle
    problem.constant('cmax',2.0,'m/s^2') #Maximum allowed centripetal acceleration
    problem.constant('eps',10,'m/s') #Error constant
    ...
    #Initial Guess
    problem.guess.setup('auto',start=[0.0,0.0,0.0,10.0,0.561,0.0], costate_guess=-0.1) #City A
