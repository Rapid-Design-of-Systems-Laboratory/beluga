...
def get_problem():
    ...
    problem = beluga.optim.Problem('heatRateWithAlfaConstraint')
    ...
    problem.constant('alfaMax', 180*pi/180, 'rad')
    ...
    problem.constraints().initial('h-h_0','m') \
                        ...
                        .path('alfaLim','alfa/alfaMax','<>',1, 'nd', start_eps=1e-4)
    ...
    problem.bvp_solver = algorithms.SingleShooting(derivative_method='fd', timeout=10, tolerance=1e-5, max_iterations=20, verbose = True, cached = False, max_error=30)
    ...
    problem.guess.setup(mode='file', filename='data-flydown.dill', step=-1, iteration=-1)

    problem.steps.add_step('bisection').num_cases(6)  \
                            .const('eps_heatRate', 1e-4)
    problem.steps.add_step('bisection').num_cases(81)  \
                            .const('qdotMax', 1200e4)

    # ^^ Saved as data-1k2-eps4-eps4.dill

    # problem.guess.setup(mode='file', filename='data-1k2-eps4-eps4.dill', step=-1, iteration=-1)
    # problem.steps.add_step('bisection').num_cases(41,spacing='log')  \
    #                         .const('eps_heatRate', 1e-6)
    # ^^ Saved as data-1k2-eps6-eps4.dill

    # problem.guess.setup(mode='file', filename='data-1k2-eps6-eps4.dill', step=-1, iteration=-1)
    # problem.steps.add_step('bisection').num_cases(41,spacing='log')  \
    #                         .const('eps_heatRate', 1e-6) \
    #                         .const('eps_alfaLim', 5e-7)
